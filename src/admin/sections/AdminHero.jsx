...
import { Sparkles, HelpCircle, Link2, Image, Share2, Save } from "lucide-react";
...
export default function AdminHero({ onSave }) {
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    // This transmits the fully modified object up to your global admin layout
    onSave("मुख्य बैनर और एक्शन स्ट्रिप (Action Strip) डेटा सफलतापूर्वक अपडेट हो गया है!");
    console.log("Updated Complete Payload:", heroData);
  };
  ...
  return (
    <form onSubmit={handleFormSubmit} className="max-w-4xl space-y-8 pb-12 selection:bg-saffron selection:text-darkbrown">
      ...
      {/* NEW BLOCK 3: ACTION STRIP & SOCIAL LINKS CONNECTORS */}
      <div className="bg-[#240a00] border border-white/5 p-6 rounded-2xl space-y-5">
        <h4 className="text-base font-semibold text-saffron tracking-wide border-b border-white/5 pb-2 flex items-center gap-2">
          <Share2 className="w-5 h-5" /> 3. सोशल मीडिया और एक्शन स्ट्रिप लिंक्स (Socials Context)
        </h4>
        <p className="text-xs text-[#f3d9b1]/40 -mt-2">
          यह सेक्शन सीधे होमपेज की हॉरिजॉन्टल एक्शन पट्टी (Action Strip) के सोशल मीडिया हैंडल्स को बदलता है।
        </p>
      ...
      {/* BLOCK 5: SWIPER SLIDER IMAGES BACKGROUND */}
      <div className="bg-[#240a00] border border-white/5 p-6 rounded-2xl space-y-5">
        <h4 className="text-base font-semibold text-saffron tracking-wide border-b border-white/5 pb-2 flex items-center gap-2">
        ...
        <div className="space-y-4">
          {heroData.sliderImages.map((imgUrl, idx) => (
            <div key={idx} className="flex gap-4 items-center bg-white/2 p-3 rounded-xl border border-white/5">
              <div className="w-12 h-12 bg-cover bg-center rounded-lg border border-white/10 shrink-0 bg-[#130500]" style={{ backgroundImage: `url(${imgUrl})` }} />
              <div className="flex-1">
                <label className="text-[10px] text-orange-200 block mb-1 font-mono">SLIDE_IMAGE_ASSET #{idx + 1}</label>
                <input
                  type="text"
                  value={imgUrl}
                  onChange={(e) => handleImageChange(idx, e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-lg py-1.5 px-3 text-cream text-xs focus:outline-none"
                />
              </div>
            </div>
          ))}
