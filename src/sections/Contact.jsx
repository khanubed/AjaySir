...
import { Phone, Mail, MapPin, Clock3, Send, PhoneCall } from "lucide-react";
...
export default function Contact() {
  ...
  const handleSubmit = (e) => {
    ...
    setForm({ name: "", mobile: "", address: "", message: "" });
  };

  if (!contactConfig) {
    return <div className="bg-lightcream py-12 min-h-[50vh]" />;
  }

  // लाइव कंफ़िगरेशन से डिटेल्स एरे जेनरेट करें
  const dynamicContactDetails = getContactDetailsArray(contactConfig);

  return (
    <section id="contact" className="bg-lightcream py-12 px-4 relative overflow-hidden">
      <motion.img
        src={diya}
        animate={{
          x: [0, -8, 6, -4, 5, 0],
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center max-w-3xl mx-auto">
          <p className="text-saffron uppercase amaranth-regular tracking-[4px] font-medium">Contact Us</p>
          ...
          <p className="text-brown be-vietnam-pro-light text-md max-sm:text-sm md:text-lg leading-relaxed mt-6">
            अपनी पूजा, पावन हवन, धार्मिक अनुष्ठान, वास्तु शांति, नवग्रह शांति और व्यक्तिगत आध्यात्मिक मार्गदर्शन के लिए हमसे संपर्क करें।
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-4 md:gap-10 mt-10 md:mt-20">
          ...
          >
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div>
                <label className="block text-darkbrown font-semibold mb-1">Full Name</label>
                <input
                  type="text"
                  name="name"
                  value={form.name}
              ...
              <div>
                <label className="block text-darkbrown font-semibold mb-1">Mobile Number</label>
                <input
                  type="tel"
                  name="mobile"
                  value={form.mobile}
              ...
              <div>
                <label className="block text-darkbrown font-semibold mb-1">Address</label>
                <input
                  type="text"
                  name="address"
                  value={form.address}
              ...
              <div>
                <label className="block text-darkbrown font-semibold mb-1">Message</label>
                <textarea
                  name="message"
                  value={form.message}
                  onChange={handleChange}
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-2">
                <button
                  type="submit"
                  className="flex-1 bg-darkbrown hover:bg-[#3b1400] text-cream rounded-2xl py-3 flex items-center justify-center gap-3 transition-all duration-300 font-semibold text-sm shadow-md"
                >
                  <Send size={16} /> Send Request
                </button>
                <a
                  href={`tel:${contactConfig.phone.replace(/\s+/g, '')}`}
                  className="flex-1 border border-brown/20 bg-white/60 hover:bg-white text-darkbrown rounded-2xl py-3 flex items-center justify-center gap-3 transition-all duration-300 font-semibold text-sm shadow-sm"
                >
          ...
          >
            <div className="relative z-10">
              <p className="uppercase tracking-[4px] text-xs text-saffron font-bold">हमसे संपर्क करें</p>
              ...
              <div className="space-y-4 mt-8">
                {dynamicContactDetails.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 transition-all duration-300">
                      ...
                      <div>
                        ...
                        <p className="text-[#f3d9b1] text-xs mt-0.5">{item.value}</p>
                      </div>
                    </div>
                  );
                })}
      ...
      <a
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-green-500 hover:scale-110 transition-all duration-300 shadow-2xl flex items-center justify-center"
      >
